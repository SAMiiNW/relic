...
export type TxKind = 'register' | 'authenticate';
...
export function useTransaction(onConfirmed?: () => void) {
        ...
        for (let i = 0; i < 5; i++) {
          try {
            ...
            const page = await fetchArtifacts(
              Math.max(0, Math.floor(Math.max(0, stats.artifacts - 1) / 20) * 20),
            );
            if (targetId) {
              result = page.find((a) => a.id === targetId) ?? null;
            } else {
  ...
  const submitRegister = useCallback(
    (
      ...
      description: string,
      onFlight?: (v: boolean) => void,
    ) => run('register', address, (c) => registerArtifact(c, title, description), null, onFlight),
    [run],
  );

  const submitAuthenticate = useCallback(
    (address: `0x${string}`, artifactId: string, evidence: string, onFlight?: (v: boolean) => void) =>
      run('authenticate', address, (c) => authenticateArtifact(c, artifactId, evidence), artifactId, onFlight),
    [run],
  );

  return { state, submitRegister, submitAuthenticate, reset };
}
