...
const ACCENT: Record<ToastKind, string> = {
  loading: 'border-foil text-foil',
  success: 'border-genuine text-genuine',
  error: 'border-forgery text-forgery',
  info: 'border-foil/40 text-parchment',
};
...
function ToastRow({ item, onDismiss }: { item: ToastItem; onDismiss: () => void }) {
  ...
  return (
    <motion.div
      ...
      className={`engraved pointer-events-auto bg-stock-800 p-4 ${ACCENT[item.kind]}`}
    >
      <div className="flex items-start gap-3">
        <Icon size={18} className={`mt-0.5 shrink-0 ${item.kind === 'loading' ? 'animate-spin' : ''}`} />
        <div className="min-w-0 flex-1">
          <p className="font-body text-sm text-parchment">{item.message}</p>
          {item.hash && (
            <a
              href={`${EXPLORER}/tx/${item.hash}`}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-1 flex items-center gap-1 font-mono text-[11px] text-muted hover:text-foil"
            >
        ...
        <button
          ...
          onClick={onDismiss}
          className="focus-ring shrink-0 text-faint hover:text-parchment"
        >
          <X size={16} />
        </button>
      </div>
